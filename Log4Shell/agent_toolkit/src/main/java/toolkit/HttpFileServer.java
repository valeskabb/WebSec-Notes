...
public class HttpFileServer {
    ...
    //返回类文件的请求数（LLM 据此来判断类是否已经下载）
    private static volatile int httpHits = 0;

    public static int getHttpHits() {
        return httpHits;
    }

    public static void resetHttpHits() {
        httpHits = 0;
    }
     ...
     * 启动 HTTP 服务器
     *
     * @param directory 恶意类所在目录
     * @param port      监听端口（仍是8888）
     */
    public static HttpServer start(String directory, int port) throws IOException {
        ...
        // daemon 线程：HttpFileServer 常被其他程序内嵌调用，不阻塞 JVM 退出
